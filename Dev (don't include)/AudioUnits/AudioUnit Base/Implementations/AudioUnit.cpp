...
AudioUnit::AudioUnit(int sampleRate_, int bufferSize_, unsigned int numInlets_, unsigned int numChannels_): Unit(sampleRate_, bufferSize_)
{
    ...
    for (auto& inlet : inlets)
    {
        ...
        for (auto& mixer : inlet->mixers)
            mixer = std::make_unique<Sum>(*this);
    }
}
...
void AudioUnit::connect(AudioUnit *unit, unsigned int inlet)
{
    ...
    for(auto channel = 0; channel < numChannels; channel++)
        inlets[inlet]->mixers[channel]->connect("Main", unit->getOutputObject(channel));
